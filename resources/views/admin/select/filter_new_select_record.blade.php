	...
	if(is_checked==true)
	  	{
            ...
            $.ajax({
		    	type: "POST",
		    	url: "{{url('/admin/selected_record/add_selected_record_new')}}",
	...
	else
	  	{
            ...
            $.ajax({
		    	type: "POST",
		    	url: "{{url('/admin/selected_record/remove_selected_record_new')}}",
		    	data: {"data" : record_id,"_token": $('#token').val()}, 
		    	cache: false,
	...
	<div class="row">
		...
		<table class="table">
		  ...
		  @foreach ($record_list as $each_record)
		    <tr>
		      <td><input type="checkbox" class="select_checkbox" name="selected_record[]" id="{{$each_record->id}}" value="{{$each_record->id}}" onClick="select_record_checkbox({{$each_record->id}})" <?php $has_record = SelectRecord::check_selected_record_new($each_record->id); if($has_record=="1"){echo "checked";}?>/></td>
